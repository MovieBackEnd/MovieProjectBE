...
public class ScreenController {
    ...
    @ApiOperation(value = "상영관 할인 정책 설정", notes = "해당 상영시간표의 할인 정책을 설정한다.")
    @PostMapping("{screen_id}/discount/policy")
    public ResponseEntity<?> setDiscountPolicy(@PathVariable Long screen_id, @Validated @RequestBody FeePolicyForm feePolicyForm, BindingResult bindingResult) {
        if(bindingResult.hasErrors()){
            List<String> errors = bindingResult.getAllErrors().stream().map(e->e.getDefaultMessage()).collect(Collectors.toList());
            return ResponseEntity.badRequest().body(errors);
        }
        ScreenDTO screenDTO = screenService.setDiscountPolicy(screen_id, feePolicyForm);
